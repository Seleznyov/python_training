...
class ContactHelper:
    ...
    def get_contact_list(self):
        ...
        if self.contact_cache is None:
            ...
            for row in driver.find_elements_by_name("entry"):
                ...
                # Читаем все телефоны и рабиваем их пердварительно указываем что это текст
    ...
    def get_contact_info_from_editpage(self, index):
        driver = self.app.driver
        self.opne_contacnt_to_edit_by_index(index)
        firstname = driver.find_element_by_name("home").get_attribute("value")
        lastname = driver.find_element_by_name("home").get_attribute("value")
    ...
    def get_contact_from_viewpage(self,index):
        driver = self.app.driver
        self.opne_contacnt_view_by_index(index)
        full_text = driver.find_element_by_id("content").text
        homephone=re.search("H: (.*)",full_text).group(1)
        mobilephone=re.search("M: (.*)",full_text).group(1)
        workphone=re.search("W: (.*)",full_text).group(1)
        secondaryphone=re.search("P: (.*)",full_text).group(1)
